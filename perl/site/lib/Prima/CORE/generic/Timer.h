...
/* methods definition */
	...
	int ( *timeout)( Handle self, Bool set, int value);
	Bool ( *get_active)( Handle self);
	Bool ( *start)( Handle self);
	void ( *stop)( Handle self);
} Timer_vmt, *PTimer_vmt;

extern PTimer_vmt CTimer;
...
extern void register_Timer_Class( void);

/* Local methods definitions */
extern void Timer_cleanup( Handle self);
extern void Timer_done( Handle self);
extern void Timer_init( Handle self, HV * profile );
extern SV * Timer_profile_default( Handle self);
extern SV * Timer_get_handle( Handle self);
extern void Timer_handle_event( Handle self, PEvent event );
extern SV * Timer_notification_types( Handle self);
extern int Timer_timeout( Handle self, Bool set, int value);
extern Bool Timer_get_active( Handle self);
extern Bool Timer_start( Handle self);
extern void Timer_stop( Handle self);
...
#undef  get_timeout
#undef  set_timeout
#define get_timeout(__hs)         timeout((__hs),0,(int)0)
#define set_timeout(__hs,__val)   timeout((__hs),1,(__val))
#undef  get_name
#undef  set_name
