...
"MSWin32-x64-multi-thread-10.0.18363.720" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.18363.720) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAENOMORE WSAEINVAL WSAEPROTOTYPE WSAEACCES WSAENETUNREACH WSAEDQUOT
	WSAEPROVIDERFAILEDINIT WSAEOPNOTSUPP WSAELOOP WSAECONNABORTED
	WSAEDESTADDRREQ WSAEPROCLIM WSAECONNRESET WSAENAMETOOLONG WSAEISCONN
	WSAEBADF WSAEADDRINUSE WSAESOCKTNOSUPPORT WSAENETRESET
	WSAEPROTONOSUPPORT WSAEINPROGRESS WSAENOBUFS WSAEADDRNOTAVAIL
	WSAENOPROTOOPT WSAEMFILE WSAESTALE WSAETIMEDOUT WSAEWOULDBLOCK
	WSAESHUTDOWN WSAEAFNOSUPPORT WSAEMSGSIZE WSAEUSERS WSAEDISCON
	WSAEINVALIDPROCTABLE WSAENOTCONN WSAEPFNOSUPPORT WSAENOTSOCK
	WSAECONNREFUSED WSAEINVALIDPROVIDER WSAETOOMANYREFS WSAEHOSTUNREACH
	WSAEHOSTDOWN WSAEFAULT WSAEINTR WSAENETDOWN WSAEREMOTE WSAEREFUSED
	WSAENOTEMPTY WSAECANCELLED WSAEALREADY
    )],
);
