...
@echo off
setlocal
set binperl=%~dp0perl\bin\perl.exe
...
echo Make I/O redirection work 
REG ADD HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer /v InheritConsoleHandles /t REG_DWORD /d 0 /f
echo Associate *.pl files with perl
assoc .pl=PerlScript
ftype PerlScript="%binperl%" "%%1" %%*
echo PATHEXT environment variable.
setx PATHEXT %PATHEXT%;.PL
REG ADD HKCU\Software\Classes\Applications\perl.exe\shell\open\command /t REG_SZ  /d """"%binperl%""" """%%1""" %%*" /f
REG QUERY HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FileExts\.pl\UserChoice /v ProgId
pause
